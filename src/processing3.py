...
class BasketRandomWalk_POP(RecoModel):
	'''
	This class build a basket sensitive random walk model on bipartite network. Supposedly the same
	technique as the Random Walk With Restart.
	'''
	def __init__(self, UI, d=0.8):
		self.UI = UI
		self.d = d
		super(BasketRandomWalk_POP, self).__init__(self.UI)
		self._buildRandomWalkScore(d)
...
class SOPRecoModel_Marco(RecoModel):
	'''
	This class contain all model information on trained data
	cf(sop)
	'''
	def __init__(self, UI, alpha=0.5, teta=0.9):
		...
		self.alpha = alpha
		self.teta = teta
		super(SOPRecoModel_Marco,self).__init__(UI)
...
class RandomWalkWithRestartRecoModel(RecoModel):
	...
	def __init__(self, UI, alpha, sim=None):
		self.UI = UI
		self.alpha = alpha
		self.sim = sim
...
class PLSARecoModel(RecoModel):
	
	def __init__(self, UI, k):
		self.UI = UI
		self.k = k
		self.maxiter = 500
		self.eps=0.01
		super(PLSARecoModel, self).__init__(UI)
